...
test('no lines visited overall', () => {
  ...
  expect(overallStatus).toEqual('You have visited 0 out of 443 stations (0%).')
})

test('7 stations visited overall', () => {
  const overallStatus = status.overall(visitedStations)
  expect(overallStatus).toEqual('You have visited 7 out of 443 stations (2%).')
})
...
test('2nd DLR station is visited', () => {
  ...
  expect(stationStatus).toEqual(`You have visited 7 out of 443 stations (2%).
Central: 1/49.
DLR: 2/45.
Northern: 3/50.
...
test('all stations visited', () => {
  ...
  expect(stationStatus).toEqual('Congratulations! You have completed all 443 stations of the Tube Challenge. Time to try the Bus Challenge?')
})

test('the all lines status check', () => {
  const allStatus = status.all(visitedStations)
  expect(allStatus).toEqual(`You have visited 7 out of 443 stations (2%).
Bakerloo: 1/25.
Central: 1/49.
Circle: 2/35.
DLR: 2/45.
